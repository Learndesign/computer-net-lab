...
int tcp_hash(struct tcp_sock *tsk)
{
	...
	{ // not only established, but all tsk that has 4-tuple.
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len)
{
	...
	{
		//睡眠的优化效果基本没啥，可以忽略
		while (tsk->send_num > 50)
		{
			sleep_on(tsk->wait_send);
		}
		int sendlen = min(len, ETH_FRAME_LEN - TCP_HEADER_LEN);
		...
		int pkt_len = sendlen + TCP_HEADER_LEN;
		char *pkt = malloc(pkt_len);
		memcpy(pkt + TCP_HEADER_LEN, buf, sendlen);
